...
> **Critical — never use `cli_exec "set simplified_<var> = <value>"` as a substitute for `set_pid_sliders`.** Plain CLI `set` only writes the slider *percentage* field; it does **not** recalculate the underlying `p_roll`/`i_roll`/`d_roll`/`f_roll`/`d_max_roll` etc. gains the FC actually flies on — that recalculation only happens via the MSP slider path (`set_pid_sliders`), the CLI command `simplified_tuning apply`, or a factory reset. Setting a `simplified_*` variable via raw CLI silently desyncs the stored percentage from the live gains; Betaflight Configurator will detect the mismatch and disable the PID tab sliders. If `set_pid_sliders` is failing (e.g. MSP timeout, dropped connection), **reconnect and retry it** rather than falling back to raw CLI — the MCP server's `cli_exec` tool now auto-runs `simplified_tuning apply` after any `set simplified_*` command as a safety net, but this is a fallback, not a substitute for the right tool.
